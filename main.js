...
function carregarDadosDoLocalStorage() {
    ...
    if (savedDespesas) {
        ...
        despesas.forEach(d => {
            adicionarDespesaNaLista(d.descricao, d.valor);
        });
    }
...
saveExpenseBtn.addEventListener('click', function () {
    ...
    if (descricao && valor > 0) {
        ...
        atualizarSaldo(); // **ADICIONADO AQUI**
        ...
        adicionarDespesaNoModal(despesa); // Mudar para passar o objeto despesa
    } else {
        alert('Por favor, preencha a descrição e o valor da despesa.');
    }
});

// Função para adicionar despesas na lista
function adicionarDespesaNoModal(despesa) {
    ...
    trashIcon.addEventListener('click', function () {
        ...
        salvarDadosNoLocalStorage(); // Salva a nova lista no Local Storage
...
saveSalaryBtn.addEventListener('click', function () {
    ...
    if (valorSalario > 0) {
        ...
        salvarDadosNoLocalStorage(); // Salva no Local Storage
    } else {
...
function atualizarListaPrincipal() {
    ...
    despesas.forEach(d => {
        ...
        const descricaoSpan = document.createElement('span');
        descricaoSpan.textContent = d.descricao;

        const valorSpan = document.createElement('span');
        valorSpan.textContent = `R$ ${d.valor.toFixed(2)}`;
        valorSpan.classList.add('value');
...
function salvarDadosNoLocalStorage() {
    ...
    localStorage.setItem('despesas', JSON.stringify(despesas)); // Salva sempre que há uma alteração
}
